%% 二维圆柱声散计算程序空间指向性分布曲线（数值结果与自由入射声场和理论解析解对比分析）20191228修改
%% 基本算法实现
%% 程序初始化与公用数据导入模块
clear all
clf
...
% plot(xx_num,yy_num,'ms','MarkerSize',8);
% plot(xx_free,yy_free,'r.','MarkerSize',16);

% 创建 plot
plot(xx_alt,yy_alt,'LineWidth',1.5,...
    'Color',[1 0 0]);

% 创建 plot
plot(xx_num,yy_num,'MarkerFaceColor',[1 0 0],...
    'MarkerSize',6,...
    'Marker','square',...
    'LineStyle','none',...
    'Color',[1 0 0]);
...
% 创建 plot
plot(xx_free,yy_free,'MarkerFaceColor',[0 0 1],...
    'MarkerSize',6,...
    'Marker','square',...
    'LineStyle','none',...
    'Color',[0 0 1]);
