...
status=zeros(1,ntot); % status数组设置各状态运动规律
                      % 注：静止规律：数值为0；（默认）
                      %     等速运动规律：数值为1；
                      %     等加速等减速规律（一半等加速，一半等减速）：数值为2；
                      %     等加速度规律：数值为3；
                      %     余弦加速度规律：数值为4；
                      % ……可添加不同规律
% 推程部分各段分界角度（单位：度）（nt-1个）
f(1)=0;
f(2)=90;
f(3)=145;
% 推程与回程转换分界角度（单位：度）（1个）
f(4)=180;
% 回程部分各段分界角度（单位：度）（nh-1个）
f(5)=270;
f(6)=330;
f(7)=360;
% f=f.*(pi/180); % 将度转化为弧度
status=[4,2,0,3,4,0];
h_target=[0,20,40,40,25,0,0]; % 设置各运动状态的目标位移(ntot+1个)
                       % 以此例说明：第一段余弦加速度状态推程需要达到的目标位移是s=20mm; fi=0°~90°
                       %             第二段等加速等减速状态推程需要达到的位移保持s=h=40mm; fi=90°~145°
                       %             第三段静止状态推程保持s=h=40mm不变; fi=145°~180°
                       %             第四段等加速度状态回程需要达到的目标位移为s=25mm; fi=180°~270°
                       %             第五段余弦加速度状态回程需要达到的位移为s=0mm; fi=270°~330°
                       %             第六段静止状态回程保持s=0mm不变; fi=330°~360°
                       %             推程分为三段，回程分为三段，一个周期为360°
...
for i=1:ntot % i表示当前处理段
    ...
    for j=1:k % j表示当前处理段中的当前点
        ...
        elseif (status(i)==1) % 等速运动规律
            ...
            ds_f(jd)=(ht/fr);
            d2s_f(jd)=0;
        elseif (status(i)==2) % 等加速等减速运动规律（一半等加速，一半等减速）
            if (j<=(k/2))
