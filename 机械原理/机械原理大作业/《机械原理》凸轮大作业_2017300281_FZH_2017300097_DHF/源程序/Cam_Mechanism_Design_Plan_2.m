...
status=zeros(1,ntot); % status数组设置各状态运动规律
                      ...
                      % ……可添加不同规律
f(1)=0;
f(2)=90; % 推程部分各段分界角度（单位：度）（nt-1个）
f(3)=180; % 推程与回程转换分界角度（单位：度）（1个）
f(4)=270; % 回程部分各段分界角度（单位：度）（nh-1个）
f(5)=360;
% f=f.*(pi/180); % 将度转化为弧度
status=[1,0,4,3];
h_target=[0,25,25,10,0]; % 设置各运动状态的目标位移
                       % 以此例说明：第一段等速状态推程需要达到的目标位移是s=h=25mm;
                       %             第二段静止状态推程需要达到的位移保持s=h=25mm不变;
                       %             第三段余弦加速度状态回程需要达到的目标位移为s=10mm;
                       %             第四段等加速度状态回程需要达到的位移为s=0mm;
