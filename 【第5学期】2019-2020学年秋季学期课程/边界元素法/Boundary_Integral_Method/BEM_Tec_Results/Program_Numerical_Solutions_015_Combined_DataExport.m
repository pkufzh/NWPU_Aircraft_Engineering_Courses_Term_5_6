%% 二维圆柱声散计算程序（解析方法+边界元方法）20191213修改
%% 基本算法实现
%% 程序初始化与公用数据导入模块
clear all
clf
...
legend('Analytical Results','Numerical Results');
title('Comparison of Analytical and Numerical Solutions of the Acoustic Field');
grid on
box on
hold off

%% 导出数据，以导入Tecplot中显示
str='Acoustic_Field_Data.dat'; % 创建Acoustic_Field_Data.dat写入声场计算结果
fid1=fopen(str,'w+'); % 以"w+"读写方式打开文件。先读后写。该文件已存在则更新；不存在则创建。
% 创建可导入Tecplot的数据云.dat文件
fprintf(fid1,'VARIABLES = "Theta","GA","GN"\n');
fprintf(fid1,'ZONE I = %d, F = POINT\n',n_ob);
for i=1:n_ob
       fprintf(fid1,'%-8.2f %-16g %-16g\n',fi_x(i)*(180/pi),GA_xy_r(i),G_xy_r(i));
end
fclose(fid1);
